...
class Application:
    ...
    def __init__(self):
        ...
        Label(self.root,text='Fichier a traiter:').grid(row=0)            #cree le texte
        self.entree1=Entry(self.root, width=10)
        self.entree1.grid(row=0,column=1)
        test=self.entree1
        Button(self.root, text='Browse...', command=(lambda x=test:x.insert(0,askopenfilename()))).grid(row=0,column=2)
        ...
        Button(self.root, text='Run', command=self.runOligator).grid(row=3,column=1)
        Quitter(self.root).grid(row=3,column=4)
        Button(self.root, text='Help', command=self.runHelp).grid(row=3,column=5)
    ...
    def runHelp(self):
        showinfo('Help','Oligator version 1.1 edition du 29 octobre 2004.\n\nCe logiciel a ete realise par Frederic Lechauve frederic_lechauve@yahoo.fr en python 2.3')
